...
def train(train_loader, model, optimizer, epoch):
    ...
    if (epoch + 1) % 5 == 0 or (epoch + 1) == opt.epoch:
        torch.save(model.state_dict(), save_path + 'EATNet-%d.pth' % epoch)
        print('[Saving Snapshot:]', save_path + 'EATNet-%d.pth' % epoch)
        file.write('[Saving Snapshot:]' + save_path + 'EATNet-%d.pth' % epoch + '\n')
...
if __name__ == '__main__':
    ...
    train_loader = get_loader(image_root, gt_root, edge_root, batchsize=8, trainsize=opt.trainsize)
